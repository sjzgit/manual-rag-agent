...
识别接口 `recognize(question, history=None)`：`history` 为会话历史消息（不含当前输入）。LLM 路径将历史作为多轮 user/assistant 消息注入 prompt（最多 6 条、单条截断 500 字符）；规则降级路径依据历史推断最近模块（关键词 / 手册核心名匹配），用于承接性提问补全与同分消歧。`simple_input` 为结合历史补全后的完整问题（如 `"实验会议室预约操作手册：那审批流程呢？"`），用于向量检索。
...
- **ADR-007 意图识别结合会话上下文**：识别输入 = 当前问题 + 最近会话历史（≤6 条、单条 ≤500 字符）。LLM 路径把历史注入多轮 messages；规则降级路径按"关键词 / 手册完整名 / 手册核心名（去"操作手册"后缀，兼容"实验室预约"类简称）"从历史推断最近模块，用于承接性提问（当前输入无关键词命中且非闲聊）补全判 precise，以及多候选同分时加权消歧；闲聊/礼貌短句不结合历史，保持拒答
